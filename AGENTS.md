...
このファイルは、このリポジトリで作業するときの Codex (Codex.ai/code) へのガイドです。
...
コラム記事執筆スキル。`disable-model-invocation: true` のため、ユーザーが明示的に呼び出した際のみ実行される（Codex による自動発火はしない）。
...
- スキル本体: `.Codex/skills/write-column/SKILL.md`
...
- スキル本体: `.Codex/skills/suggest-column/SKILL.md`
